...
import com.grapeshot.halfnes.ButtonChomper;
...
public class ControllerImpl implements ControllerInterface, KeyListener {
    ...
    private ButtonChomper chomps;

    public ControllerImpl(final java.awt.Component parent, final Preferences prefs, final int controllernum, ButtonChomper bc) {
        chomps = bc;
    	...
    	// This static initialization of two controllers is a bad approach.
    	// I'll rewrite this at some point.
        if ((controllernum != 0) && (controllernum != 1)){
            throw new IllegalArgumentException("controllerNum must be 0 or 1");
        }
        
        this.controllernum = controllernum;
        chomps.print_to_log("Loading Controller: ID " + controllernum);
        this.prefs = prefs;
        setButtons();
        this.parent = parent;
    ...
    public void keyPressed(final KeyEvent arg0) {
        ...
        final int keypressed = arg0.getKeyCode();
        
        chomps.print_to_log("Key Pressed: " + keypressed);
        
        if (!m.containsKey(keypressed)) {
            return;
        }
        //enable the corresponding bit to the key
        controllerbyte |= m.get(keypressed);
    ...
    public void keyReleased(final KeyEvent arg0) {
        ...
        chomps.print_to_log("Key Released: " + keypressed);
...
//        }
    ...
    // For input components that pull continuous float values.
    // This isn't the best way to do this, but I'm leaving it
    // as is for right now. It's here because the ISO components
    // are super jumpy and trigger when you do anything near them.
    private double con_in_thresh = 0.25;
                ...
                if (gameController != null) {
                    ...
                    while (!Thread.interrupted()) {
                        ...
                        while (queue.getNextEvent(event)) {
                            ...
                            // Event Test Battery
                            // I customized the log messages for the Xbox 360 controller.
                            // I'll abstract this out later to allow for other devices.
                            ...
                            if(component.getIdentifier() == Component.Identifier.Axis.POV){
                            	if(event.getValue() == 0.25){
                            		// Up = On
                            		gamepadbyte |= BIT4;
                            		gamepadbyte &= ~(BIT5 | BIT6 | BIT7);
                            		
                            		chomps.print_to_log(gameController.getPortNumber(), event.toString(), "");
                            	}
                            	
                            	else if(event.getValue() == 0.50){
                            		// Right = On
                            		gamepadbyte |= BIT7;
                            		gamepadbyte &= ~(BIT4 | BIT5 | BIT6);
                            	...
                            	/*
                            	// The diagnoal directions on the d-pad.
                            	else if(event.getValue() == 0.125){
                            		// Left = On, Up = On
                            		gamepadbyte |= (BIT6 | BIT4);
                            		gamepadbyte &= ~(BIT5 | BIT7);
                            		
                            		chomps.print_to_log(gameController.getPortNumber(), event.toString(), "");
                            	}else if(event.getValue() == 0.375){
                            		// Right = On, Up = on
                            		gamepadbyte |= (BIT7 | BIT4);
                            		gamepadbyte &= ~(BIT5 | BIT6);
                            		
                            		chomps.print_to_log(gameController.getPortNumber(), event.toString(), "");
                            	}else if(event.getValue() == 0.625){
                            		// Right = On, Down = On
                            		gamepadbyte |= (BIT7 | BIT5);
                            		gamepadbyte &= ~(BIT4 | BIT6);
                            		
                            		chomps.print_to_log(gameController.getPortNumber(), event.toString(), "");
                            	}else if(event.getValue() == 0.875){
                            		// Left = On, Down = On
                            		gamepadbyte |= (BIT6 | BIT5);
                            		gamepadbyte &= ~(BIT4 | BIT7);
                            	...
                            	*/
                            		...
                            		gamepadbyte &= ~(BIT4 | BIT5 | BIT6 | BIT7);
                            		
                            		chomps.print_to_log(gameController.getPortNumber(), event.toString(), "");
                            	}
                            }
                            else if(component.getIdentifier() == Component.Identifier.Axis.X){
                            	if (event.getValue() > con_in_thresh) {
                                    // Left = Off, Right = On
                            		gamepadbyte |= BIT7;
                                    gamepadbyte &= ~BIT6;

                                	chomps.print_to_log(gameController.getPortNumber(), event.toString(), "");
                                } else if (event.getValue() < (con_in_thresh * -1)) {
                                	// Left = On, Right = Off
                                	gamepadbyte |= BIT6;
                                    gamepadbyte &= ~BIT7;
                            	...
                            	if (event.getValue() > con_in_thresh) {
                                    ...
                                    gamepadbyte &= ~(BIT5 | BIT4);

                                	//chomps.print_to_log(gameController.getPortNumber(), event.toString(), "");
                                }
                            } 
                            else if(component.getIdentifier() == Component.Identifier.Axis.RX){
                            	if (event.getValue() > con_in_thresh) {
                                    // Left = Off, Right = On
                            		gamepadbyte |= BIT7;
                                    gamepadbyte &= ~BIT6;
                            ...
                            // Stuff I haven't caught yet. Should eventually be empty.
                            else{
                            	gamepadbyte = 0;
    ...
    private static Controller[] getAvailablePadControllers() {
        ...
        // Check the usable controllers (gamepads or joysticks with at least 2 axis and 2 buttons)
    ...
    public final void setButtons() {
        ...
        if (controllers.length > controllernum) {
            ...
            // Not sure why this would output to stderr. 
